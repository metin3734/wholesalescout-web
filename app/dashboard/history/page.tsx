...
export default function SearchHistoryPage() {
  ...
  if (jobs.length === 0) {
    return (
      <div style={{ background:'#fff', borderRadius:'12px', border:'1px solid #e2e8f0', padding:'4rem 2rem', textAlign:'center' }}>
        ...
        <div style={{ fontWeight:600, color:'#0f172a', marginBottom:'0.3rem' }}>Henüz arama geçmişi yok</div>
        <div style={{ color:'#94a3b8', fontSize:'0.82rem', marginBottom:'1.25rem' }}>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
        ...
        {[
          ...
          <div key={s.label} style={{ background:'#fff', borderRadius:'10px', border:'1px solid #e2e8f0', padding:'1rem 1.25rem' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'0.4rem' }}>
              <span style={{ fontSize:'0.7rem', color:'#94a3b8', textTransform:'uppercase', letterSpacing:'0.05em', fontWeight:600 }}>{s.label}</span>
              <span style={{ fontSize:'1rem' }}>{s.icon}</span>
            </div>
            <div style={{ fontWeight:700, fontSize:'1.5rem', color:'#0f172a' }}>{s.value}</div>
          </div>
        ))}
      </div>
      ...
      {/* Bulk action bar */}
      {selectedIds.size > 0 && (
        <div style={{ display:'flex', alignItems:'center', gap:'0.75rem', padding:'0.6rem 1rem', background:'#fef2f2', border:'1px solid #fecaca', borderRadius:'8px' }}>
          <span style={{ fontSize:'0.78rem', color:'#b91c1c', fontWeight:600 }}>{selectedIds.size} arama seçili</span>
          <button
          ...
          <button
            ...
            style={{ fontSize:'0.73rem', color:'#64748b', background:'none', border:'none', cursor:'pointer' }}
          >
            İptal
          </button>
        </div>
      ...
      {Object.entries(groups).map(([date, dayJobs]) => (
        <div key={date}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'0.75rem', marginBottom:'0.6rem' }}>
            <div style={{ fontWeight:700, fontSize:'0.78rem', color:'#64748b', textTransform:'uppercase', letterSpacing:'0.05em', whiteSpace:'nowrap' }}>
              {date}
            </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem' }}>
            {dayJobs.map((job) => {
              ...
              return (
                <div key={job.id} style={{ background: isSelected ? '#f0f7ff' : '#fff', borderRadius:'10px', border: isSelected ? '1px solid #bfdbfe' : '1px solid #e2e8f0', padding:'0.85rem 1.25rem', display:'flex', alignItems:'center', gap:'0.75rem', opacity: isDeleting ? 0.4 : 1, transition:'all 0.15s' }}>
                  ...
                  {/* Time */}
                  <div style={{ minWidth:'44px', fontSize:'0.7rem', color:'#94a3b8', flexShrink:0 }}>{time}</div>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontWeight:600, fontSize:'0.83rem', color:'#0f172a', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                      {job.file_name}
                    </div>
                  ...
                  {job.status === 'processing' && (
                    <div style={{ minWidth:'80px', flexShrink:0 }}>
                      ...
                      <div style={{ fontSize:'0.62rem', color:'#94a3b8', marginTop:'0.2rem', textAlign:'right' }}>{pct}%</div>
                    </div>
                  )}
                  ...
                  {job.status === 'completed' && (
                    <div style={{ display:'flex', gap:'0.3rem', flexShrink:0 }}>
                      {job.result_csv && (
                        <a href={`/api/download/${job.id}?format=csv`} style={{ padding:'0.22rem 0.5rem', border:'1px solid #e2e8f0', borderRadius:'6px', color:'#475569', fontSize:'0.65rem', fontWeight:500, textDecoration:'none' }}>CSV</a>
                      )}
                      {job.result_xlsx && (
